...
def get_summoner_recent_matches(account_id):
    """
    ...
    recent_games_info = get_response(BASE_URL + 'match/v3/matchlists/by-account/%s/recent' % account_id)['matches']
    recent_games = []
...
def get_summoner_recent_rankeds(account_id):
    """
    given an account id, return the following data
    "matches": [
        {
    ...
    recent_games_info = get_response(BASE_URL + 'match/v3/matchlists/by-account/%s?endIndex=20&beginIndex=0' % account_id)['matches']
    recent_games = []
    for game in recent_games_info:
        data = {
            'lane': game['lane'],
...
def get_several_matches_info(games_list):
    """
    ...
    for game in games_list:
        game_info = get_response(BASE_URL + 'match/v3/matches/%s' % game['gameId'])
...
def get_current_game_info(summoner_id):
    ...
    """
    current_game = {}
    current_game_info = get_response(BASE_URL + "spectator/v3/active-games/by-summoner/%s" % summoner_id)
    ...
    for participant in current_game_info['participants']:
        participant_data = {
            ...
            "runes": [session.query(Runes).get(rune['runeId']).toJson(2, count=rune["count"]) for rune in participant['runes']],
            "masteries": [session.query(Masteries).get(mastery['masteryId']).toJson(2, count=mastery["rank"]) for mastery in participant['runes']],
            "spell1": session.query(SummonerSpells).get(participant['spell1Id']).toJSon(2),
            "spell2": session.query(SummonerSpells).get(participant['spell2Id']).toJSon(2),
...
if __name__ == '__main__':
    get_current_game_info(67511124)
